...
$(function(){
    $("#ware_item_find1").click(function(){
		...
		//加表头
		$("#ware_item_find_table").append("<th>告警号</th><th>组件名</th><th>持续时间</th><th>最后时间</th><th>次数</th><th>参数</th><th>服务器类型</th><th>服务类型</th><th>所在机房</th><th>所属组</th><th>操作</th>");
		...
		    success: function(data) {
				if(data['success']){
					...
					{ 		
						...
						$("#ware_item_find_table").append("<tr><td>"+js[i].message_id+" </td><td>"+js[i].assembly_id+" </td><td>"+js[i].continue_time+" </td><td>"+js[i].last_time+" </td><td>"+js[i].times+" </td><td>"+js[i].parameters+" </td><td>"+js[i].server_type+" </td><td>"+js[i].service_type+" </td><td>"+js[i].room+" </td><td>"+js[i].group+" </td><td>"+deal+" </td></tr>");
					}
				}else{
					alert(data['message']);
				}
...
$(function(){
	$("#login_enter").click(function(){
		...
		    success: function(data) {
				if(data['success']){
					window.location.href = 'manager2/manager2';
				}else{
					alert(data['message']);
					}
			    }
